			...
			try:
				for idx in con_idx:
					...
					if factor == 'PM2.5' or factor == 'PM10':
						if len(np_row[np_row<2])>0 or  len(np_row[np_row>120])>0:
							raise Error()
						#print(min(np_row) ,float(row[left+time_range]) ,max(np_row))
						y = np.float(row[left+time_range])
						x2= np.float(np_row[-2])
						x1= np.float(np_row[-1])
						minx = np.float(min(np_row))
						maxx = np.float(max(np_row))
	
						if not ( minx - 7  <= y <= maxx + 7 ) and abs(y - x2 + 2 * x1) > 5 :
							raise Error("OAO")
						#print(np_row)
					elif factor == 'CO':
						pass
		...
		for i in range(N-time_range+1):
			...
			if i == N-time_range:
				...
				for _ in range(len(data)):
					if data[_] in [nani[0] , nani[-1]]:
						# special value judge:
						if _ == 0 and abs(data[_] - data[0]) > 5:
							data[0] = data[1]
							print("{0:*>6s}".format(str(data[_])) , end=',')
					
						elif _ != len(data)-1 and abs( (data[_+1] + data[_-1])/2 -data[_]) > 5:
							data[_] = (data[_-1] + data[_-2]) /2
							print("{0:*>6s}".format(str(data[_])) , end=',')
