...
embed = nn.Embedding(len(words), 300)
embed.weight.data.copy_(torch.load('./W.pt'))
embed.eval()
embed.cuda()
...
def train(xys, inv):
    ...
    xys = PackedSequence(embed(xys.data), xys.batch_sizes)
    xys = encoder(xys)
...
for t in range(TOTAL_EPOCH):
    ...
    torch.save(encoder.state_dict(), 'model5.pt')
